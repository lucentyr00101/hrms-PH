...
const errorHandler = (error) => {
  if (error.response) {
    ...
    if (error.response.status === 401 && refreshToken) {
      ...
      notification.error({
        message: 'Unauthorized',
        description: 'You are no longer allowed to access this site. You will be logged out automatically <5 seconds countdown>.'
      })
      if (token) {
        store.dispatch('Logout').then(() => {
          setTimeout(() => {
...
request.interceptors.request.use(async (config) => {
  const token = storage.get(ACCESS_TOKEN)
  const refreshToken = storage.get(REFRESH_TOKEN)
  const noRefresh = ['refresh_token', 'login', 'logout']
  // 如果 token 存在
  // 让每个请求携带自定义 token 请根据实际情况自行修改
  if (!token && refreshToken && !noRefresh.some(apiName => config.url.includes(apiName))) {
    await store.dispatch('RefreshToken')
  } else if (token && refreshToken && config.url.includes('logout')) {
